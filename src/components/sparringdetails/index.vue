   ...
   <div class="returnBtn" @click="routerGo()">返回</div>
	<div class="exhibition">
		<swiper :options="swiperOption" ref="mySwiper">
			<swiper-slide class="lists" :key = ind v-for="(imageList,ind) in detailsstr.image_list"><img :src=imageList alt="" style="width:100%; height:220px;">
			</swiper-slide>
			<div class="swiper-pagination" slot="pagination"></div>  
		</swiper>
			<img v-if='detailsstr.video_url==""' src="@/static/images/lcon/videoicon.png" class="videoicon" @click='open1()' style="display:none;">
		<img v-else src="@/static/images/lcon/videoicon.png" class="videoicon" @click='open1()'>
		<video class='videos' style='width:100%; display:none;' :src=detailsstr.video_url
			></video>
	</div>
	<div class="d_main">
		<div class="brief">
		...
		<ul class="nain_icon">
			<li v-for='(list,inde) in detailsstr.tag_list'>{{list}}</li>
		</ul>
		<div class="d_score">
		...
		<dl>
			...
			<dd>
				...
				<p @click='voicesFn(detailsstr.audio_duration)'>
					<img class="s_voices" v-lazy=this.srcimg1>
					 <audio class='audioref' id="audiorefs" :src=detailsstr.audio>
	               </audio>
	               <span>
						{{detailsstr.audio_duration}}"
					</span>
				</p>
			</dd>
		</dl>
...
<div class="v-modal" @click="open()" tabindex="0" style="z-index: 2000; display: none">
 ...
 GetParam(url, id) {
      ...
      if(verif == "" || verif==null){
       ...
       var aId =  this.GetParam(aidUrl,"code"),
        ...
        this.$http.post("/h5/esport-api/v1/baidu/authcode_to_uid",{'auth_code':aId,"redirect_url":"m.pw.dui1dui.com/#/sparringdetails?id="+redirect},{emulateJSON: true}).then(
              (response)=>{
    ...
    alertFn(tit){
      ...
      let audiorefs = $('#audiorefs'),
      	  svoices = document.getElementsByClassName('s_voices')[0];
      ...
      if (this.isgn == false) {
        ...
        if(audioDuration == 0){
          ...
          clearInterval(this.timers);
          audiorefs[0].pause();
         svoices.src = this.srcimg1;
         
         this.isgn = false;
        }
    },
    playFn(duration){
    	let svoices = document.getElementsByClassName('s_voices')[0];
  	...
  	open1(){
  		console.log($('.videos').attr('src'))
  		let str = $("<video class='videon' style='width: 100%; max-height:71%; position:fixed;right: 0;left: 0;top:0;bottom: 0;margin:auto;'></video>").attr('src',$('.videos').attr('src'))
  		$('.pading').html(str);
  		$('.v-modal').css('display',"block");
