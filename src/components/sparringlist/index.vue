<template>
  ...
  <v-scroll :on-refresh="onRefresh" :on-infinite="onInfinite">
    <div class="kings_list">
      <div class="lings_dl" v-for="(listarr,ind) in codelits" @click="getcode('/sparringdetails',listarr.user_game_service_id)">
        <dl>
            <dt>
          ...
          <div class="lingImg">
            <img src="@/static/images/lcon/voices.png" class="voicescne" @click='voicesFn(listarr)'>
...
methods:{
    ...
    getList(){
      ...
      this.$http.post("/h5/esport-api/v1/service/list_play_game_service",{'menu_code':this.playtitcode,'page_no':1,'page_size':this.num},{emulateJSON: true}).then(
              ...
              let voicescne = document.getElementsByClassName('voicescne'),
  ...
  parentclick(code,ind) {
    ...
    // api.Login({'parant_code':parent}).then((res)=>{
    //     console.log(res)
    // });
  ...
  condeFn(playtitcode){
    ...
    voicesFn(obj){
      ...
      clearInterval(this.timers);
        ...
        let audioref = document.getElementsByClassName('audioref'),
            voicescne = document.getElementsByClassName('voicescne');
    ...
    playFn(ind,duration){
      ...
      this.timers = setInterval(()=>{
        $('.voicescne').eq(ind).attr("src",arr[n])
        n++;
        if (n == 3) {
          n = 0;
        };
       ...
       this.durations = setInterval(()=>{
        ...
        if(duration<=0){
           clearInterval(this.timers);
           $('.voicescne').eq(ind).attr("src",this.srcimg)
        }
      },1000);
    }
  },
